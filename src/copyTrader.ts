...
export class CopyTrader {
  ...
  private copied = new Map<string, CopiedPositionState>();

  private lastFetchErrorAt = new Map<string, number>();

  private lastPositionsFingerprint = new Map<string, string>();

  private lastRemoteSize = new Map<string, number>();

  private pending = new Map<
    string,
    { detectedAt: number; expiresAt: number; wallet: string }
  >();
  ...
  private displayStatusSummary() {
    const now = Date.now();
    if (now - this.lastStatusUpdate < 60000) return; // Une fois par minute
  ...
  private lastSummarySnapshot: {
    ...
    copied: number;
    error?: string;
  } = { positions: 0, pending: 0, copied: 0, error: undefined };

  private running = false;

  constructor(private polymarket: PolymarketService) {}

  private posKey(p: { wallet?: string; marketId: string; outcome: OutcomeSide }) {
  ...
  private async loop(): Promise<void> {
    ...
    try {
      if (config.debug) log.debug("---- tick ----");
      await this.tick();
    } catch (err) {
  ...
  private async tick() {
        ...
        if (!config.debug && (status === 404 || status === 400)) {
          if (now - last > 60_000) {
            log.info(
              ...
              "Aucune position ou endpoint indispo (404/400)",
            );
    ...
    for (const [key, pending] of Array.from(this.pending.entries())) {
      if (!activeKeys.has(key)) {
        ...
        if (config.debug) log.debug({ key }, "Pending annulé (position absente)");
        continue;
      }
      if (now >= pending.expiresAt) {
        this.pending.delete(key);
        if (config.debug) log.debug({ key }, "Pending expiré sans drawdown");
      }
    }
  ...
  private logPositionsOnceOnChange(wallet: string, positions: RemotePosition[]) {
  ...
  private logSummary(positionsCount: number) {
    // Silence: pas de résumé périodique
  }

  private async handleWalletPositions(
    wallet: string,
    ...
    for (const pos of positions) {
      ...
      if (this.copied.has(copyKey)) continue;
  ...
  private async computeBudget(entryPrice: number): Promise<{ usd: number; size: number }> {
    const balance = await this.polymarket.getUsdcBalance();
    if (balance <= 0) throw new Error("Balance USDC insuffisante");
  ...
  private async tryEnterCopy(pos: RemotePosition) {
    const price = await this.getReferencePrice(pos);
    if (!this.isTradablePrice(price)) {
      if (config.debug) {
        log.debug(
          { market: pos.marketId, tokenId: pos.tokenId, reason: "midpoint hors plage tradable" },
          "Marché expiré ou sans OB → ignoré",
        );
      }
      return;
    }
  ...
  private async getReferencePrice(pos: RemotePosition): Promise<number> {
    try {
      const mid = await this.polymarket.getMidPrice(pos.marketId, pos.tokenId, pos.outcome);
      if (this.isTradablePrice(mid.midpoint)) return mid.midpoint;
      return 0;
    } catch (err) {
  ...
  private async manageCopiedPositions() {
    for (const state of this.copied.values()) {
      try {
        ...
        if (!this.isTradablePrice(price)) continue;
  ...
  private async sellPortion(state: CopiedPositionState, percent: number) {
  ...
  private async closePosition(state: CopiedPositionState, reason: string) {
